...
// later: have fun overengineering these URLs with parameterized year
let amazonOrderHistYearURLs = [
	'https://www.amazon.com/gp/your-account/order-history?orderFilter=year-2020',
	'https://www.amazon.com/gp/your-account/order-history?orderFilter=year-2021',
];
...
// start when #start
let startBtn = document.getElementById('start-scraping-btn');
startBtn.classList.add('ready');
startBtn.onclick = function(element) {
    console.log('onclick 0', element)
    startBtn.classList.remove('ready');
    startBtn.classList.add('scraping');
    startBtn.innerHTML = 'scraping...';

	let starterURL = amazonOrderHistYearURLs[0];
	console.log('go to', starterURL)
    // create a new tab (unless already on project page URL) and go to Amazon
	chrome.tabs.query({
        active: true,
        currentWindow: true
    }, async function(tabs) {
        ...
        if (activeTab.url.indexOf(PROJECTURL) != 0) {
			...
			activeTab.url = starterURL;
		}
    });


    // // 2021 order history -- TODO: handle pagination after 10 orders
    // let url = 'https://www.amazon.com/gp/your-account/order-history?orderFilter=year-2021';
...
// I think use long lived connections: https://developer.chrome.com/docs/extensions/mv3/messaging/#connect
// and pass a message from the content script to the runtime
...
// 				// documentation: https://developer.chrome.com/docs/extensions/mv3/messaging/
